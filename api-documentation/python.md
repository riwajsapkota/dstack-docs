...
The `push()` method **creates** **a frame** in the stack, **commits** and **pushes** the data in a single operation. If you want to use interactive plots, you can use the `frame()`, `add()`, and then `push()`

{% hint style="warning" %}
`push_frame()` method has now been deprecated and been replaced by the `push()` method
{% endhint %}
...
{% hint style="warning" %}
 `commit()` method has now been deprecated and been replaced by the `add()`method.
{% endhint %}
